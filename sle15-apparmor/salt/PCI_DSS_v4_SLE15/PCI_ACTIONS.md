...
Generated 2026-06-06T17:53:57+00:00 from `ssg-sle15-ds.xml` (profile `xccdf_org.ssgproject.content_profile_pci-dss-4`, MAC: apparmor).
...
| `package_dhcp_removed`<br><sub>Uninstall DHCP Server Package</sub> | medium | 2.2.4, 2.2 | remove package `dhcp-server` |
...
| `package_rsh-server_removed`<br><sub>Uninstall rsh-server Package</sub> | high | 2.2.4, 2.2 | remove package `rsh-server` |
| `package_rsh_removed`<br><sub>Uninstall rsh Package</sub> | unknown | 2.2.4, 2.2 | remove package `rsh` |
| `package_sudo_installed`<br><sub>Install sudo Package</sub> | medium | 2.2.6, 2.2 | install package `sudo` |
| `package_talk-server_removed`<br><sub>Uninstall talk-server Package</sub> | medium | 2.2.4, 2.2 | remove package `talk-server` |
| `package_talk_removed`<br><sub>Uninstall talk Package</sub> | medium | 2.2.4, 2.2 | remove package `talk` |
...
| `package_xinetd_removed`<br><sub>Uninstall xinetd package if not used by network services</sub> | low | 2.2.4, 2.2 | remove package `xinetd` |
| `package_ypbind_removed`<br><sub>Remove NIS Client</sub> | unknown | 2.2.4, 2.2 | remove package `ypbind` |
| `package_ypserv_removed`<br><sub>Uninstall ypserv Package</sub> | high | Req-2.2.2, 2.2.4, 2.2 | remove package `ypserv` |
...
| `rpm_verify_permissions`<br><sub>Verify and Correct File Permissions with RPM</sub> | high | Req-11.5, 11.5.2 | operational cmd: `rpm -qa | xargs -r -n1 rpm --setperms 2>/dev/null || true` |
...
| `accounts_password_warn_age_login_defs`<br><sub>Set Password Warning Age</sub> | medium | Req-8.2.4, 8.3.9, 8.3 | line in `/etc/login.defs`: `PASS_WARN_AGE 7` |
...
| `set_password_hashing_algorithm_libuserconf`<br><sub>Set Password Hashing Algorithm in /etc/libuser.conf</sub> | medium | Req-8.2.1, 8.3.2, 8.3 | line in `/etc/libuser.conf`: `crypt_style = sha512` |
| `set_password_hashing_algorithm_logindefs`<br><sub>Set Password Hashing Algorithm in /etc/login.defs</sub> | medium | Req-8.2.1, 8.3.2, 8.3 | line in `/etc/login.defs`: `ENCRYPT_METHOD SHA512` |
...
| Rule | Sev | PCI-DSS | Action |
|---|---|---|---|
| `cracklib_accounts_password_pam_dcredit`<br><sub>Set Password Strength Minimum Digit Characters</sub> | medium | Req-8.2.3, 8.3.6, 8.3 | operational cmd: `f=/etc/pam.d/common-password; grep -qE "^[[:space:]]*password[[:space:]]+requisite[[:sp…` |
| `cracklib_accounts_password_pam_lcredit`<br><sub>Set Password Strength Minimum Lowercase Characters</sub> | medium | Req-8.2.3, 8.3.6, 8.3 | operational cmd: `f=/etc/pam.d/common-password; grep -qE "^[[:space:]]*password[[:space:]]+requisite[[:sp…` |
| `cracklib_accounts_password_pam_minlen`<br><sub>Set Password Minimum Length</sub> | medium | Req-8.2.3, 8.3.6, 8.3 | operational cmd: `f=/etc/pam.d/common-password; grep -qE "^[[:space:]]*password[[:space:]]+requisite[[:sp…` |
| `cracklib_accounts_password_pam_retry`<br><sub>Set Password Retry Limit</sub> | medium | Req-8.1.6, Req-8.1.7, 8.3.4, 8.3 | operational cmd: `f=/etc/pam.d/common-password; grep -qE "^[[:space:]]*password[[:space:]]+requisite[[:sp…` |
...
| `audit_rules_enable_syscall_auditing`<br><sub>Remove Default Configuration to Disable Syscall Auditing</sub> | medium | CM-6(b), CM-6.1(iv), SRG-OS-000480-GPOS-00227, SLES-15-030820, SLES-15-750450450, SV-234981r991589_rule | operational cmd: `sed -ri 's/^([[:space:]]*-a[[:space:]]+task,never)/#\1/' /etc/audit/rules.d/*.rules` |
...
| `audit_rules_login_events_tallylog`<br><sub>Record Attempts to Alter Logon and Logout Events - tallylog</sub> | medium | Req-10.2.3, 10.2.1.3, 10.2.1, 10.2 | manage `/etc/audit/rules.d/pci-audit_rules_login_events_tallylog.rules` (mode 0640) |
...
## Firewall (iptables loopback rules) (`firewall`)

| Rule | Sev | PCI-DSS | Action |
|---|---|---|---|
| `set_ipv6_loopback_traffic`<br><sub>Set configuration for IPv6 loopback traffic</sub> | medium | Req-1.3, 1.4.1, 1.4 | iptables.append `` |
| `set_loopback_traffic`<br><sub>Set configuration for loopback traffic</sub> | medium | Req-1.3, 1.4.1, 1.4 | iptables.append `` |
