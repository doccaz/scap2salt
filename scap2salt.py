...
%changelog
* {rpmdate} scap2salt <noreply@example.com> - {version}-{release}
...
Both paths are already in the MLM server's Salt file roots, so no extra config
is needed. (Restart the spacewalk/uyuni services or just refresh the page if the
formula does not appear immediately.)
...
def emit_package(outdir, meta, mac, version="1.0.0", release="0"):
    ...
    write(spec_path, SPEC_TEMPLATE.format(
        ...
        prof=meta["prof"], mac=mac, rpmdate=rpmdate))
    ...
    write(os.path.join(pkgdir, "README.md"), PKG_README.format(
        name=PKG_NAME, version=version, release=release, formula=FORMULA,
        prof=meta["prof"], mac=mac, ts=meta["ts"]))
...
def main():
    ...
    ap.add_argument("--pkg-version", default="1.0.0",
