...
def _resolve_formatted_output(bash):
    ...
    if not pfv_m:
        return None
    fmt, val = pfv_m.group(1), shell_resolve(pfv_m.group(2), bash)
    if "$" in val:
        return None
    # The separator between the two %s in the printf format is significant:
    # login.defs uses "%s %s" (space) but /etc/default/useradd uses "%s=%s".
    sep_m = re.search(r"%s(.*?)%s", fmt)
    sep = sep_m.group(1) if sep_m else " "
    return f"{key}{sep}{val}"


# Existing-account aging rules: CaC iterates /etc/shadow and applies chage/passwd
# per non-compliant user. No declarative Salt primitive, so emit a guarded
# operational cmd.run mirroring the awk filter + command (idempotent via onlyif).
_ACCT_EXISTING = {
    "accounts_password_set_max_life_existing": (
        "var_accounts_maximum_age_login_defs",
        r'/^[^:]+:[^!*]/ && ($5 > var || $5 == "")', "passwd -q -x {v}"),
    "accounts_password_set_warn_age_existing": (
        "var_accounts_password_warn_age_login_defs",
        r'($6 < var || $6 == "") && $2 ~ /^\$/', "chage --warndays {v}"),
    "accounts_set_post_pw_existing": (
        "var_account_disable_post_pw_expiration",
        r'($7 > var || $7 == "") && $2 ~ /^\$/', "chage --inactive {v}"),
}


@mapper
def m_accounts_existing(r):
    spec = _ACCT_EXISTING.get(r.short)
    if not spec:
        return None
    varname, filt, cmd = spec
    vm = re.search(rf"{re.escape(varname)}='([^']*)'", r.bash or "")
    if not vm:
        return None
    v = vm.group(1)
    awk = f"awk -v var={v} -F: '({filt}) {{print $1}}' /etc/shadow"
    st = SaltState(
        r.short, "cmd.run",
        [("name", f'{awk} | while IFS= read -r u; do {cmd.format(v=v)} "$u"; done'),
         ("onlyif", f'test -n "$({awk})"')],
        "accounts", r,
    )
    st.operational = True
...
def m_sudoers(r):
    ...
    if not r.short.startswith("sudo_"):
        return None
    bash = r.bash or ""
    if r.short == "sudo_require_authentication":
        # CaC comments out every NOPASSWD / !authenticate line in the sudoers
        # files. Operational; guarded so it only runs when an uncommented one
        # exists. NOTE: this removes passwordless sudo — if your management agent
        # relies on NOPASSWD, leave the 'sudo' category unchecked.
        pat = "^[[:space:]]*[^#[:space:]].*(NOPASSWD|!authenticate)"
        st = SaltState(
            "sudo_require_authentication", "cmd.run",
            [("name", 'for f in /etc/sudoers /etc/sudoers.d/*; do [ -e "$f" ] || '
                      "continue; sed -ri '/" + pat + "/ s/^/# /' \"$f\"; done"),
             ("onlyif", "grep -rEq '" + pat + "' /etc/sudoers /etc/sudoers.d/ 2>/dev/null")],
            "accounts", r,   # opt-in/default-off: removing NOPASSWD can lock out agents
        )
        st.operational = True
        return st
...
CATEGORIES = ["sysctl", "packages", "rpm", "services", "permissions",
              "kernel_modules", "sshd", "lineinfile", "accounts", "pam", "sudo",
              "audit", "dconf", "coredump", "grub", "firewall", "limits",
              "aide", "mounts", "mac", "misc"]
# Categories that DEFAULT OFF (opt-in). These mutate existing accounts/access in
# ways that can lock out users or management agents — e.g. expiring existing
# passwords (chage; an expired account can't even be fixed with chage afterwards)
# or removing NOPASSWD sudo. Operators must enable them deliberately.
OPT_IN_CATEGORIES = {"accounts"}
...
CAT_LABELS = {
    ...
    "accounts":      "Invasive auth hardening: password aging + sudo re-auth (opt-in)",
...
# Deploy helper: copy the formula from the RPM payload into the LIVE MLM podman
# volumes under /var/lib/containers/storage (the dirs the uyuni-server container
# mounts as /srv/salt and /srv/formula_metadata). Run by %post on RW hosts and
# by a boot-time oneshot on SL Micro — see %post for why both are needed.
install -d %{{buildroot}}%{{_libexecdir}}
cat > %{{buildroot}}%{{_libexecdir}}/deploy-%{{formula}}.sh <<'DEPLOY'
#!/bin/sh
# Auto-generated by scap2salt — deploy %{{formula}} into the MLM container volumes.
formula="%{{formula}}"
_meta="" _salt=""
...
[ -n "$_meta" ] || _meta=/var/lib/containers/storage/volumes/srv-formulametadata/_data
[ -n "$_salt" ] || _salt=/var/lib/containers/storage/volumes/srv-salt/_data
# Only a containerised MLM has these volumes; on a traditional server this is a
# no-op (the /usr/share/salt-formulas payload + file_roots is the mechanism).
[ -d "$_meta" ] && [ -d "$_salt" ] || exit 0
mkdir -p "$_meta/$formula" "$_salt/$formula"
cp -a /usr/share/salt-formulas/metadata/"$formula"/. "$_meta/$formula/"
cp -a /usr/share/salt-formulas/states/"$formula"/. "$_salt/$formula/"
echo "deployed $formula to MLM container volumes (states: $_salt/$formula)"
DEPLOY
chmod 0755 %{{buildroot}}%{{_libexecdir}}/deploy-%{{formula}}.sh

install -d %{{buildroot}}%{{_unitdir}}
cat > %{{buildroot}}%{{_unitdir}}/deploy-%{{formula}}.service <<'UNIT'
[Unit]
Description=Deploy %{{formula}} Salt formula into MLM container volumes
After=network-online.target
Wants=network-online.target
ConditionPathIsDirectory=/var/lib/containers/storage/volumes
[Service]
Type=oneshot
RemainAfterExit=yes
ExecStart=%{{_libexecdir}}/deploy-%{{formula}}.sh
[Install]
WantedBy=multi-user.target
UNIT
# Vendor enablement via a packaged wants-symlink in /usr — systemd reads
# /usr/lib/systemd/system/multi-user.target.wants/ at boot. This enables the
# oneshot WITHOUT a `systemctl enable` in %post, which is unreliable inside the
# transactional-update chroot on SL Micro (where /etc changes don't surface as
# expected). The symlink ships in the snapshot's /usr and goes live on reboot.
install -d %{{buildroot}}%{{_unitdir}}/multi-user.target.wants
ln -sf ../deploy-%{{formula}}.service \
   %{{buildroot}}%{{_unitdir}}/multi-user.target.wants/deploy-%{{formula}}.service
...
%{{_libexecdir}}/deploy-%{{formula}}.sh
%{{_unitdir}}/deploy-%{{formula}}.service
%{{_unitdir}}/multi-user.target.wants/deploy-%{{formula}}.service

%post
# The formula must land in the LIVE /var/lib/containers/storage volumes, not the
# host's /usr/share (which the uyuni-server container never reads).
# - Traditional / RW containerised host: the podman volumes are live during
#   %post, so this deploys immediately — usable without a reboot.
# - SL Micro / transactional: %post runs in the transactional-update chroot,
#   where /var/lib/containers is a *shadowed* snapshot copy, so this is a no-op.
#   The deploy-%{{formula}}.service oneshot (vendor-enabled via the shipped
#   wants-symlink above) runs the same copy on the next boot, against the live
#   storage. A reboot is required anyway to activate a transactional install.
%{{_libexecdir}}/deploy-%{{formula}}.sh || true
systemctl start deploy-%{{formula}}.service >/dev/null 2>&1 || true

%preun
# On final removal (not upgrade) stop the oneshot and clean the volume copies.
# (The wants-symlink is removed automatically as a packaged file.)
if [ $1 -eq 0 ]; then
    systemctl stop deploy-%{{formula}}.service >/dev/null 2>&1 || true
    _meta="" _salt=""
    if command -v podman >/dev/null 2>&1; then
        _meta=$(podman volume inspect srv-formulametadata --format '{{{{.Mountpoint}}}}' 2>/dev/null || true)
        _salt=$(podman volume inspect srv-salt --format '{{{{.Mountpoint}}}}' 2>/dev/null || true)
...
The RPM payload installs to:

    /usr/share/salt-formulas/metadata/{formula}/   (form.yml, metadata.yml)
    /usr/share/salt-formulas/states/{formula}/      (the Salt state tree)
    /usr/libexec/deploy-{formula}.sh                 (volume-deploy helper)
    /usr/lib/systemd/system/deploy-{formula}.service (boot-time oneshot)

The MLM/Uyuni Salt master only reads `/srv/salt` + `/srv/formula_metadata`, which
on a containerised server are **podman volumes** under
`/var/lib/containers/storage/volumes/srv-{{salt,formulametadata}}/_data`. So the
package does not just drop files in `/usr/share` — it also copies the formula
into those live volumes:

**Containerised MLM 5.x on SL Micro (read-only / transactional root):**
...
`%post` runs in the transactional-update chroot, where the container storage is
*shadowed* and cannot be written — so on first boot the bundled
`deploy-{formula}.service` oneshot (vendor-enabled via a shipped
`multi-user.target.wants` symlink) copies the formula into the live volumes. To
deploy **without** waiting for a reboot, run the helper directly after install:

    sudo /usr/libexec/deploy-{formula}.sh

**Containerised MLM on a writable root (RW host):** `sudo zypper install
--allow-unsigned-rpm ./{name}-{version}-{release}.noarch.rpm` — here `%post`
deploys to the live volumes immediately, no reboot needed.

**Traditional (non-containerised) MLM / Uyuni:** `/usr/share/salt-formulas/` must
be in the server's `file_roots` (the volume-deploy is a no-op without the
podman volumes). If the formula does not appear, add the path and restart the
spacewalk/uyuni services, or refresh the web page.

## Use

1. In the Web UI go to a **system** or **system group** -> **Formulas**.
2. Tick **PCI-DSS v4 Hardening**, then Save.
3. Open the new **{formula}** sub-tab; toggle categories as needed, Save. This
   writes the `{formula}:` pillar the states read.
